        ...
        <div>
                    ...
                    <TableBody>
                        {props.data.data.map((row, index) => (
                            <TableRow key={row.id}>
                                ...
                                <TableCell align="left">{row.discuss_with_employee_field.name}</TableCell>
          ...
          open={viewDetailsButton} onClose={() => {Closeviewbutton() }} >
                        ...
                        <Grid container justify='flex-start' direction='row' alignItems='center' spacing={2}>
                         ...
                         { Message.map((row) => <Grid item xs={12}>
                                <Card style={{ minWidth: 400, marginTop: 10 }} elevation={4} variant="outlined" >
                                    <CardContent>
                                        <Typography variant="h5" component="h2">{row.initiated_by_field}</Typography>
                    ...
                    <DialogActions style={{ padding: 15 }}>
                        <Button color="primary" variant="contained" disabled={Message[0].msgCategory_field==="ApprovalRequests"} onClick={() => { setaddNewMessageDialog(true) }}>
                            Add new message
                            </Button>
                        <Button color="secondary" variant="contained" onClick={() => Closeviewbutton()}>
                            Back to Inbox
                            </Button>
                    </DialogActions></>}
                </Dialog>
            ...
            <div>
                <Dialog open={addNewMessageDialog} onClose={() => setaddNewMessageDialog(false)} >
                    ...
                    <DialogActions>
                        <Button variant='contained' color="primary" onClick={() => addMessages(Message[0].id)}>
                            Submit
                        </Button>
                        <Button variant='contained' color="secondary" onClick={() => setaddNewMessageDialog(false)}>
                            Cancel
                        </Button>
                    </DialogActions>
                </Dialog>
